...
export type SelectedLetter = { id: number; title: string };

export default function useSuspenseGetLetter() {
  return useSuspenseQuery<SelectedLetter>(letterQueryOptions());
}

export const letterQueryOptions = (): UseQueryOptions<SelectedLetter> => ({
  queryKey: [QueryKeys.Letter],
  // queryFn: () => http.get("/selected"),
  queryFn: () => Promise.resolve({ id: 1, title: text }),
});
